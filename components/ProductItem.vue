    ...
    <div class="course-item bg-light">
      ...
      <div class="d-flex border-top">
        <small class="flex-fill text-center border-end py-2"><i class="fa fa-toilet text-primary me-2"></i>
          <NuxtLink :to="`/hersteller/${product.brand.toLowerCase()}/`" :title="product.brand">{{ product.brand }}
          </NuxtLink>
        </small>
        <small class="flex-fill text-center border-end py-2"><i class="fa text-primary me-2" v-if="category"></i>
          <NuxtLink v-if="product.category" :to="`/kategorie/${product.category.toLowerCase()}/`"
...
export default {
  ...
  data() {
    ...
    const affiliate = config.affiliates.find((affiliate) => affiliate.name === this.product.brand)

    if (affiliate?.active) {
      var url = new URL(this.product.shopLink)
      url.searchParams.append(affiliate.parameter.key, affiliate.parameter.value)
      this.product.shopLink = url.toString()
    }
